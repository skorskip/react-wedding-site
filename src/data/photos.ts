import austinPano from "../assets/images/png/austin_pano.png";
import amsterdamPano from "../assets/images/png/amsterdam_pano.png";
import kyotoPano from "../assets/images/png/kyoto_pano.png";
import zakopanePano from "../assets/images/png/zakopane_pano.png";
import brugePano from "../assets/images/png/bruge_pano.png";
import austinSquare from "../assets/images/png/austin_square.jpg";
import makiFamilyPhoto from "../assets/images/png/maki_family_photo.jpg";
import glennArborPano from "../assets/images/png/glenn_arbor_pano.png";
import amsterdamSquare from "../assets/images/png/amsterdam_square.jpg";
import glennArborSquare from "../assets/images/png/glenn_arbor_square.jpg";
import kyotoSquare from "../assets/images/png/kyoto_square.jpg";
import zakopaneSquare from "../assets/images/png/zakopane_square.jpg";
import engagement1 from "../assets/images/png/engagement_1.jpg";
import engagement2 from "../assets/images/png/engagement_2.jpg";
import engagement3 from "../assets/images/png/engagement_3.jpg";

import austinPanoWebp from "../assets/images/webp/austin_pano.webp";
import amsterdamPanoWebp from "../assets/images/webp/amsterdam_pano.webp";
import kyotoPanoWebp from "../assets/images/webp/kyoto_pano.webp";
import zakopanePanoWebp from "../assets/images/webp/zakopane_pano.webp";
import brugePanoWebp from "../assets/images/webp/bruge_pano.webp";
import austinSquareWebp from "../assets/images/webp/austin_square.webp";
import makiFamilyPhotoWebp from "../assets/images/webp/maki_family_photo.webp";
import glennArborPanoWebp from "../assets/images/webp/glenn_arbor_pano.webp";
import amsterdamSquareWebp from "../assets/images/webp/amsterdam_square.webp";
import glennArborSquareWebp from "../assets/images/webp/glenn_arbor_square.webp";
import kyotoSquareWebp from "../assets/images/webp/kyoto_square.webp";
import zakopaneSquareWebp from "../assets/images/webp/zakopane_square.webp";
import engagement1Webp from "../assets/images/webp/engagement_1.webp";
import engagement2Webp from "../assets/images/webp/engagement_2.webp";
import engagement3Webp from "../assets/images/webp/engagement_3.webp";

export const PHOTO_ENUM = {
    AUSTIN_PANO : "austinPano",
    AUSTIN_SQUARE : "austinSquare",
    AMSTERDAM_PANO : "amsterdamPano",
    AMSTERDAM_SQUARE : "amsterdamSquare",
    KYOTO_PANO : "kyotoPano",
    BRUGE_PANO : "brugePano",
    KYOTO_SQUARE : "kyotoSquare",
    ZAKOPANE_PANO : "zakopanePano",
    ZAKOPANE_SQUARE : "zakopaneSquare",
    MAKI_FAMILY_PHOTO : "makiFamilyPhoto",
    GLENN_ARBOR_PANO : "glennArborPano",
    GLENN_ARBOR_SQUARE : "glennArborSquare",
    ENGAGEMENT_1 : "engagement1",
    ENGAGEMENT_2 : "engagement2",
    ENGAGEMENT_3 : "engagement3"
}
export const photos = {
    austinPano : {
        png : austinPano,
        webp : austinPanoWebp
    },
    austinSquare : {
        png : austinSquare,
        webp : austinSquareWebp
    },
    amsterdamPano : {
        png : amsterdamPano,
        webp : amsterdamPanoWebp
    },
    amsterdamSquare : {
        png : amsterdamSquare,
        webp : amsterdamSquareWebp
    },
    kyotoPano : {
        png : kyotoPano,
        webp : kyotoPanoWebp
    },
    kyotoSquare : {
        png : kyotoSquare,
        webp : kyotoSquareWebp
    },
    zakopanePano : {
        png : zakopanePano,
        webp : zakopanePanoWebp
    },
    zakopaneSquare : {
        png : zakopaneSquare,
        webp : zakopaneSquareWebp
    },
    makiFamilyPhoto : {
        png : makiFamilyPhoto,
        webp : makiFamilyPhotoWebp
    },
    glennArborPano : {
        png : glennArborPano,
        webp : glennArborPanoWebp
    },
    glennArborSquare : {
        png : glennArborSquare,
        webp : glennArborSquareWebp
    },
    brugePano : {
        png : brugePano,
        webp: brugePanoWebp
    },
    engagement1 : {
        png : engagement1,
        webp : engagement1Webp
    },
    engagement2 : {
        png : engagement2,
        wbep : engagement2Webp
    },
    engagement3 : {
        png : engagement3,
        wbep : engagement3Webp
    }
} as any